...
export default function ClientsList() {
  ...
  async function addClient(e) {
    ...
    e.preventDefault();
    clientEmailsArray.includes(clientEmail)
      ? setError("Client Exist")
      : await set(
          newPostRef,
    ...
    <>
      <div>
        ...
        >
          ...
          {error && (
            <Alert
              variant="danger"
              style={{ width: "345px", position: "absolute", top: "150%" }}
            >
              {error}
            </Alert>
          )}
        ...
        <table style={{ width: "600px", textAlign: "center" }}>
          ...
          {clients.map((anObjectMapped, index) => {
            return (
              <>
                <tr
                  key={`${anObjectMapped.clientName}_{anObjectMapped.clientEmail}`}
                >
                  <td>{index + 1}</td>
                  <td>{anObjectMapped[1].clientName}</td>
                  <td>{anObjectMapped[1].clientEmail}</td>
                  <td>
                    <Button
                      className="mx-2"
                      variant="primary"
                      onClick={
                        (handleEdit = () => {
                          editClientEmailRef.current.value =
                            anObjectMapped[1].clientEmail;
                          editClientNameRef.current.value =
      ...
      <div className="edit-container">
        ...
        <input
          type="text"
          className="text"
          ref={editClientNameRef}
          required
        ></input>
        <input
          type="email"
          className="text"
          ref={editClientEmailRef}
          required
        ></input>
        <input
          type="submit"
          className="editclientbtn"
